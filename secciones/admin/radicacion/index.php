...
<br />
<br/>
<a href="../../../secciones/admin/opciones_radicacion/index.php" class="btn btn-primary btn-lg">Atrás</a>
              
<form  action="http://localhost/BotPlussFB/peticiones/radicacioncontroller.php" method="post" enctype="multipart/form-data">
        ...
        <div class="container">
          <div class="row">
            <div class="col">
              ...
              <input type="text" class="form-control" id="num_radicado" name="num_radicado" value="<?php echo $nuevoRadicadoNumero; ?>" readonly>
            </div>
            <div class="col">
              <label for="num_escritura" class="form-label">Número escritura</label>
              <input type="text" class="form-control" id="num_escritura" name="num_escritura">
            </div>
            <div class="col">
              <div class="mb-3">
                ...
                <select class="form-select" id="acto_notarial" name="acto_notarial">
                    ...
                    <?php foreach ($actosNotariales as $acto): ?>
                        <option value="<?php echo htmlspecialchars($acto['nombre']); ?>">
                            <?php echo htmlspecialchars($acto['nombre']); ?>
                        </option>
                    <?php endforeach; ?>
                  </select>
              </div>
          ...
          <div class="row">
            ...
            <div class="col">
            <div class="mb-3">
                <label for="digitador" class="form-label">Digitador</label>              
                <input type="text" class="form-control" id="digitador" name="digitador" readonly>
                <input type="hidden" id="digitador_id" name="digitador_id">
            </div>
        </div>       
            <div class="col">
              <div class="mb-3">
                ...
                <select class="form-select" id="proyecto" name="proyecto">
                  <option selected>Seleccione...</option>
                  <option value="casa de la loma">N/A</option>
...
document.getElementById('add-matricula').addEventListener('click', function() {
  ...
  input.placeholder = 'Ingresar matrícula';
...
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    ...
    $(document).ready(function() {
        // Obtener el digitador activo con menor puntaje al cargar la página
        $.ajax({
            url: 'http://localhost/BotPlussFB/peticiones/obtener_usuario_digitador.php',
            method: 'GET',
            success: function(response) {
                try {
                    const digitador = JSON.parse(response);
                    if (digitador && digitador.usuario && digitador.id_usuario) {
                        $('#digitador').val(digitador.usuario);
                        $('#digitador_id').val(digitador.id_usuario);
                    } else if (digitador && digitador.error) {
                        $('#digitador').val(digitador.error);
                    } else {
                        $('#digitador').val('No disponible');
                    }
                } catch (e) {
                    $('#digitador').val('Error en el formato de la respuesta JSON');
                }
            },
            error: function() {
                $('#digitador').val('Error al obtener digitador');
            }
        });
    });
    </script>
      ...
      <form action="upload.php" method="post" enctype="multipart/form-data">
        <div class="table-responsive-sm">
          <div class="container">
            ...
            <div class="row">
              <div class="col">
              <button type="submit" class="btn btn-primary btn-lg" name="action" value="radicar">Radicar</button>
           <!--   <button type="submit" class="btn btn-primary btn-lg" name="action" value="guardar_borrador">Guardar como borrador</button>-->
              <a href="../../../secciones/admin/opciones_radicacion/index.php" class="btn btn-primary btn-lg">Atrás</a>
              </div>
            </div>
          </div>
